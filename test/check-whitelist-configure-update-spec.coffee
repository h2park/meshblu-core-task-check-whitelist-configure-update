http = require 'http'
CheckWhitelistConfigureUpdate = require '../'

describe 'CheckWhitelistConfigureUpdate', ->
  beforeEach ->
    @whitelistManager =
      checkConfigureUpdate: sinon.stub()

    @sut = new CheckWhitelistConfigureUpdate
      whitelistManager: @whitelistManager

  describe '->do', ->
    describe 'when called with a valid job', ->
      beforeEach (done) ->
        @whitelistManager.checkConfigureUpdate.yields null, true
        job =
          metadata:
            auth:
              uuid: 'green-blue'
    ...
    describe 'when called with a valid job without a from', ->
      ...
      it 'should call the whitelistmanager with the correct arguments', ->
        expect(@whitelistManager.checkConfigureUpdate).to.have.been.calledWith emitter: 'green-blue', subscriber: 'bright-green'
    ...
    describe 'when called with a job that with a device that has an invalid whitelist', ->
      beforeEach (done) ->
        @whitelistManager.checkConfigureUpdate.yields null, false
        job =
          metadata:
            auth:
              uuid: 'puke-green'
    ...
    describe 'when called and the checkConfigureUpdate yields an error', ->
      beforeEach (done) ->
        @whitelistManager.checkConfigureUpdate.yields new Error "black-n-black"
        job =
          metadata:
            auth:
              uuid: 'puke-green'
